...
class MCP9808Graph(octoprint.plugin.StartupPlugin, octoprint.plugin.SettingsPlugin):
	...
	def on_after_startup(self):
		...
		self.smbus.write_byte_data(self.i2c_addr, MCP9808_REG_RESOLUTION,
								   self.sensor_precision)  # High precision +0.0625 degC, 0x03(03)
...
def read_temp(self):
	currtemp = None
	try:
		data = self.smbus.read_i2c_block_data(self.i2c_addr, MCP9808_REG_AMBIENT_TEMP,
											  2)  # 2-byte temperature (temp-MSB, temp-LSB) from reg 0x05
